...
def test_swap_best_by_category():
    ...
    assert result,"Expected Truthy but got Falsy"
    assert len(tai.inventory) == 3, f"Length: {len(tai.inventory)}, Expected 3"
    assert len(jesse.inventory) == 3, f"Length: {jesse(tai.inventory)}, Expected 3"
...
def test_swap_best_by_category_reordered():
    ...
    assert len(tai.inventory) == 3, f"Length: {len(tai.inventory)}, Expected 3"
    assert len(jesse.inventory) == 3, f"Length: {jesse(tai.inventory)}, Expected 3"
    assert tai.inventory == [item_b, item_a, item_f], f"{tai.inventory}, Expected: [item_b, item_a, item_f]"
    assert jesse.inventory == [item_e, item_d, item_c], f"{jesse.inventory}, Expected: [item_e, item_d, item_c]"
...
def test_swap_best_by_category_no_match_is_false():
    ...
    assert not result, "Expected Falsy but got Truthy"
    assert len(tai.inventory) == 3, f"Length: {len(tai.inventory)}, Expected 3"
    assert len(jesse.inventory) == 3, f"Length: {jesse(tai.inventory)}, Expected 3"
    assert tai.inventory == [item_a, item_b, item_c], f"{tai.inventory}, Expected: [item_a, item_b, item_c]"
    assert jesse.inventory == [item_d, item_e, item_f], f"{jesse.inventory}, Expected: [item_d, item_e, item_f]"
...
def test_swap_best_by_category_no_other_match_is_false():
    ...
    assert len(tai.inventory) == 3, f"Length: {len(tai.inventory)}, Expected 3"
    assert len(jesse.inventory) == 3, f"Length: {jesse(tai.inventory)}, Expected 3"
    assert tai.inventory == [item_c, item_b, item_a], f"{tai.inventory}, Expected: [item_c, item_b, item_a]"
    assert jesse.inventory == [item_f, item_e, item_d], f"{jesse.inventory}, Expected: [item_f, item_e, item_d]"
